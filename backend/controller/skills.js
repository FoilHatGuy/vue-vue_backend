...
module.exports = {
    ...
    async getId(id) {
        return db.query(`SELECT * FROM SKILLS WHERE skills.skill_id = $1`, [id])
    },

    async getIdUsers(id) {
        return db.query(`SELECT u.id, u.name, u.surname, u.patronymic FROM user_x_skill as s_a
LEFT join
(select users.user_id, users.name, users.surname, users.patronymic from users) as u
ON user = u.id WHERE s_a.skill = $1`, [id])
    }
}