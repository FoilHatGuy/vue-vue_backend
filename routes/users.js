...
function init(config) {
    ...
    router.post('/:id/edit', async (req, res, next) => {
        try {
            next()
            let user = ctrl.update(req.params.id, req.body.user.name, req.body.user.surname, req.body.user.patronymic, req.body.user.department, req.body.user.position)
            ...
            } else user.then((r) => res.status(200).send(r))
        } catch (err) {
